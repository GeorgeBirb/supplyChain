App = {
    ...
    App.contracts.SupplyChain.deployed().then(function (instance) {
        ...
        return Promise.all([
            ...
            supplyChainInstance.transferOwner(),
        ]);
    }).then(function ([candidatesCount, transferShipmentsCount, contractOwner, productsCount, transferOwner]) {
        ...
        if (transferOwner.toLowerCase() === App.account.toLowerCase()) {
            $("#updateBtn").show();
            $("#transferForm").show();
        } else {
            $("#updateBtn").hide();
